...
namespace BookBarn.Data.Repositories
{
    public  class ReviewCumRatingRepo : IReviewCumRatingRepo
    {
            ...
            {
                return db.ReviewCumRatings.Include(r => r.book).Include(r => r.user).Where(r => r.book.BookID == bookId);
            }
            ...
            public List<ReviewCumRating> GetReviewCumRatings(string type)
            {
                if (type == "positive")
                {
                    return db.ReviewCumRatings.Include(r => r.book).Include(r => r.user).Where(rr => rr.Rating > 3).ToList();
                }
                ...
                {
                    return db.ReviewCumRatings.Include(r => r.book).Include(r => r.user).Where(rr => rr.Rating == 1).ToList();
                }
                else if (type == "two")
                {
                    return db.ReviewCumRatings.Include(r => r.book).Include(r => r.user).Where(rr => rr.Rating == 2).ToList();
                }
                else if (type == "three")
                {
                    return db.ReviewCumRatings.Include(r => r.book).Include(r => r.user).Where(rr => rr.Rating == 3).ToList();
                ...
                {
                    return db.ReviewCumRatings.Include(r => r.book).Include(r => r.user).Where(rr => rr.Rating == 4).ToList();
                }
                else
                {
                    return db.ReviewCumRatings.Include(r => r.book).Include(r => r.user).Where(rr => rr.Rating == 5).ToList();
                }
            }
            ...
            public ReviewCumRating GetReviewCumRatingBasedOnUser(int bookId, string userName)
            {
                ReviewCumRating reviewCumRating = (from r in db.ReviewCumRatings.Include(r => r.user).Include(r => r.book) where r.book.BookID == bookId && r.user.Username == userName select r).FirstOrDefault();
                if (reviewCumRating == null)
                {
                    return null;
                }
